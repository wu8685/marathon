...
private[impl] class KillServiceActor(
  ...
  override def receive: Receive = {
    case KillUnknownTaskById(taskId, promise) =>
      // TODO(PODS): we never receive an InstanceChanged for an unknown task, therefore the promise is never completed!
      // this was broken before and must be fixed.
      // TODO(PODS): We could publish a TerminalUnknownEvent when we receive a terminal update for an unknown instance
      // and process that
